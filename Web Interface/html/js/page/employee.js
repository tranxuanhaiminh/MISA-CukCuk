...
function initEvents() {
    ...
    var options = $('.misa-cbx-option');
    $.each(options, function (index, option) {
        $(option).on('click', selectCbx.bind(this))
    })

    // Khi bấm ra ngoài combobox
    $(window).on('click', function (e) {
        $.each($('.misa-cbx'), function (index, cbx) {
            if (!cbx.contains(e.target)) {
                $(cbx).removeClass('misa-cbx-opened');
            }
        })
    });
...
/**
 * Format combobox
 * CreatedBy: TXHMinh(25/07/2021)
 */
function toggleCbx() {
    $(this).find('.misa-cbx').toggleClass('misa-cbx-opened');
}
...
function selectCbx() {
    if (!$(this).hasClass('misa-cbx-selected')) {
        ...
        $(this).closest('.misa-cbx').find('.misa-cbx-select')[0].value = $(this)[0].innerText;
    }
}